...
abstract class LinkGeneratingProvider(val baseURI: URI?, val resolveAnchors: Boolean = false) :
    GeneratingProvider {
    final override fun processNode(
        visitor: HtmlGenerator.HtmlGeneratingVisitor,
        text: String,
        node: ASTNode
    ) {
    ...
    open fun renderLink(
        ...
        visitor.consumeTagOpen(
            node,
            "a",
            "href=\"${makeAbsoluteUrl(info.destination)}\"",
...
open class ReferenceLinksGeneratingProvider(
...
open class ImageGeneratingProvider(linkMap: LinkMap, baseURI: URI?) :
