...
import kmark.parser.markerblocks.impl.ParagraphMarkerBlock
...
abstract class MarkerProcessor<T : MarkerProcessor.StateInfo>(
    ...
    protected abstract fun populateConstraintsTokens(
    ...
    open fun createNewMarkerBlocks(
        pos: LookaheadText.Position,
        productionHolder: ProductionHolder
    ): List<MarkerBlock> {
    ...
    fun processPosition(pos: LookaheadText.Position): LookaheadText.Position? {
        ...
        if (pos.offsetInCurrentLine == -1
            ...
            val delta =
                stateInfo.nextConstraints.getCharsEaten(pos.currentLine) - pos.offsetInCurrentLine
            if (delta > 0) {
    ...
    private fun calculateNextPosForExistingMarkers(pos: LookaheadText.Position): Int {
        val result =
            markersStack.lastOrNull()?.getNextInterestingOffset(pos) ?: pos.nextLineOrEofOffset
        return if (result == -1)
            Int.MAX_VALUE
        else
            result
    ...
    private fun processMarkers(pos: LookaheadText.Position): Boolean {
        ...
        while (index > 0) {
            ...
            val markerBlock = markersStack[index]
    ...
    private fun applyProcessingResult(
        index: Int,
        markerBlock: MarkerBlock,
        processingResult: MarkerBlock.ProcessingResult
    ) {
    ...
    private fun closeChildren(index: Int, childrenAction: MarkerBlock.ClosingAction) {
        if (childrenAction != MarkerBlock.ClosingAction.NOTHING) {
            ...
            while (latterIndex > index) {
                val result = markersStack[latterIndex].acceptAction(childrenAction)
    ...
    open class StateInfo(
        val currentConstraints: MarkdownConstraints,
