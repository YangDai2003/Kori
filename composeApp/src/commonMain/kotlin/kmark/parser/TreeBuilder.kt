...
abstract class TreeBuilder @ExperimentalApi constructor(
    ...
    fun buildTree(production: List<SequentialParser.Node>): ASTNode {
        ...
        for (i in events.indices) {
            ...
            flushEverythingBeforeEvent(
                event,
                if (markersStack.isEmpty()) null else markersStack.peek().second
            )
    ...
    protected abstract fun createASTNodeOnClosingEvent(
        event: MyEvent,
        currentNodeChildren: List<MyASTNodeWrapper>,
        isTopmostNode: Boolean
    ): MyASTNodeWrapper

    protected abstract fun flushEverythingBeforeEvent(
        event: MyEvent,
    ...
    protected class MyEvent(
        val position: Int,
        val timeClosed: Int,
        val info: SequentialParser.Node
    ) : Comparable<MyEvent> {
        ...
        override fun compareTo(other: MyEvent): Int {
            ...
            if (isStart() == other.isStart()) {
                val positionDiff =
                    info.range.first + info.range.last - (other.info.range.first + other.info.range.last)
                if (positionDiff != 0) {
    ...
    protected class MyASTNodeWrapper(
        val astNode: ASTNode,
