...
class EmphasisLikeParser(private vararg val parsers: DelimiterParser) : SequentialParser {
    ...
    private fun collectDelimiters(
        tokens: TokensCache,
        iterator: TokensCache.Iterator
    ): ArrayList<DelimiterParser.Info> {
    ...
    private fun balanceDelimiters(delimiters: ArrayList<DelimiterParser.Info>) {
        ...
        for ((closerIndex, closer) in delimiters.withIndex()) {
            ...
            val minOpenerIndex =
                openersBottom[closer.marker]!![(if (closer.canOpen) 3 else 0) + (closer.length % 3)]
            ...
            while (openerIndex > minOpenerIndex) {
                ...
                if (opener.canOpen && opener.closerIndex < 0 && !violatesRuleOfThree(
                        opener,
                        closer
                    )
                ) {
     ...
     */
    private fun violatesRuleOfThree(
        opener: DelimiterParser.Info,
        closer: DelimiterParser.Info
    ): Boolean {
