...
interface MarkerBlock {
    ...
    class ProcessingResult internal constructor(
        val childrenAction: ClosingAction,
        val selfAction: ClosingAction,
        val eventAction: EventAction
    ) {

        companion object {
            val PASS: ProcessingResult = ProcessingResult(
                ClosingAction.NOTHING,
                ClosingAction.NOTHING,
                EventAction.PROPAGATE
            )
            val CANCEL: ProcessingResult =
                ProcessingResult(ClosingAction.NOTHING, ClosingAction.NOTHING, EventAction.CANCEL)
            val DEFAULT: ProcessingResult =
                ProcessingResult(ClosingAction.DEFAULT, ClosingAction.DONE, EventAction.PROPAGATE)
        }
