...
import kmark.html.InlineHolderGeneratingProvider
...
internal class CheckedListItemGeneratingProvider : SimpleTagProvider("li") {
    ...
    override fun processNode(
        ...
        if (checkBoxElement != null) {
            ...
            inputHtml =
                "<input type=\"checkbox\" class=\"task-list-item-checkbox\"$checkedString disabled />"
        } else {
            listItemClass = null
            inputHtml = ""
        }
        ...
        for (child in node.children) {
            ...
            if (!flushedInput) {
                if (child.type == MarkdownElementTypes.PARAGRAPH) {
                    SubParagraphGeneratingProvider(isLoose, inputHtml).processNode(
                        visitor,
                        text,
                        child
                    )
    ...
    private class SubParagraphGeneratingProvider(
        val wrapInParagraph: Boolean,
...
/**
 * Special version of [kmark.flavours.commonmark.CommonMarkFlavourDescriptor.CodeSpanGeneratingProvider],
...
internal class MathGeneratingProvider(private val inline: Boolean = false) : GeneratingProvider {
    override fun processNode(
        ...
        val output = nodes.joinToString(separator = "") { it.getTextInNode(text) }
            .trimSurroundingBackticksAndWhitespaces()
...
internal class TablesGeneratingProvider : GeneratingProvider {
    ...
    private fun populateRow(
        ...
        val parityAttribute =
            if (rowNumber > 0 && rowNumber % 2 == 0) "class=\"intellij-row-even\"" else null
        ...
        for (child in node.children.filter { it.type == GFMTokenTypes.CELL }.withIndex()) {
            ...
            val alignmentAttribute =
                if (alignment.isDefault) null else "align=\"${alignment.htmlName}\""
