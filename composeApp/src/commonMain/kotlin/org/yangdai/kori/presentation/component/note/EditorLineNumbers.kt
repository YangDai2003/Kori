...
fun EditorLineNumbers(
    ...
    val textMeasurer = rememberTextMeasurer(actualLinePositions.size)
    ...
    val requiredWidth by remember(actualLinePositions.size, textStyle, density) {
        derivedStateOf {
            ...
            with(density) {
                textMeasurer.measure(maxDigitsText, textStyle, maxLines = 1).size.width.toDp()
            }
        }
    }
    ...
    Canvas(
        ...
        val firstVisibleLineIndex = actualLinePositions
            .binarySearchBy(scrollOffset) { it.second }
        ...
        for (index in firstVisibleLineIndex until actualLinePositions.size) {
            ...
            if (lineY > scrollOffset + size.height) break

            // 获取行高来更精确地判断是否可见
            val lineNumberText = (index + 1).toString()
            val textLayoutResult = textMeasurer.measure(lineNumberText, textStyle, maxLines = 1)
